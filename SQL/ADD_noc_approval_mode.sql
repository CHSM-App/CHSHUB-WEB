/*
 * ADD: how many of a society's officers have to approve a NOC.
 *
 * Why a setting
 * -------------
 * A NOC request goes to every office the society has — admin, secretary and
 * chairman, whichever of them exist. What is not fixed is how many have to
 * answer before the certificate can be written, and it cannot be: this is one
 * product across many societies and their bye-laws differ.
 *
 * Neither choice is safe as a hard rule:
 *
 *   All of them — a society where the admin account and the secretary are the
 *   same person is asked to approve twice, from two logins. Worse, several of
 *   the societies on this database carry three admin and three secretary
 *   accounts; requiring all seven means their requests are never approved at
 *   all.
 *
 *   Any one — in a society whose committee genuinely reviews a sale or
 *   mortgage NOC together, one officer could issue it alone.
 *
 * So the society says which it wants. 'Any' is the default because it is the
 * one that always terminates: a society that never opens this setting can
 * still issue certificates.
 *
 * Where it lives
 * --------------
 * On account_setting, beside noc_signatories, which is where the society's
 * other NOC choices already are.
 *
 * Safe to re-run.
 */
...
IF COL_LENGTH('dbo.account_setting', 'noc_approval_mode') IS NULL
BEGIN
    -- 'Any' or 'All'. Text rather than a lookup table: the set is fixed by
    -- the app and a society cannot add to it.
    ALTER TABLE dbo.account_setting
        ADD noc_approval_mode NVARCHAR(10) NULL;

    PRINT 'account_setting.noc_approval_mode added.';
END
ELSE
    PRINT 'account_setting.noc_approval_mode already exists - no change made.';
GO

/*
 * sp_noc_request 'Update_Status' reads the setting.
 *
 * The decision was hard-coded twice over: first as "all approvers must agree",
 * copied from sp_vendor_bills, and then as "any one is enough". Neither is the
 * society's own rule, so the branch now asks account_setting which it is.
 *
 * A society with no account_setting row, or a NULL in the column, gets 'Any' —
 * the choice that always terminates. Anything else is treated as 'All' only
 * when it says so exactly, so a typo in the column cannot deadlock a society's
 * requests.
 *
 * Rejection is outside the setting: one refusal settles the request either
 * way. An officer who says no has raised an objection the others agreeing
 * cannot answer, while an officer who says yes has only added a voice.
 *
...
PRINT 'sp_noc_request updated - approval mode now read from account_setting.';
GO
