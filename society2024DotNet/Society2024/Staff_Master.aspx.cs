...
namespace Society
{
    public partial class Staff_Master : System.Web.UI.Page
    {
        ...
        public string runproc_save(string operation)
        {
            ...
            Staff.Email = txt_email.Text == "" ? "N/A" : txt_email.Text;
            Staff.Date_Of_Join = Convert.ToDateTime(txt_doj.Text);
            if (role_id.Value != "")
                Staff.role_id = Convert.ToInt32(role_id.Value);
            Staff.role = categoryBox.Text;
            Staff.Salary = Convert.ToDecimal(txt_Salary.Text);
        ...
        private void runproc_staff(string operation)
        {
            ...
            txt_Salary.Text = result.Salary.ToString();
        ...
        protected void isStaffExist(object sender, EventArgs e)
        {
            string StaffName = txt_name.Text;
            string SoceityID = Session["society_id"].ToString();
            string Operation = "isStaffExist";
            var Exist = bL_Staff.StaffExist(StaffName, SoceityID, Operation);
            lblNameExist.Visible = !Exist;
        }
